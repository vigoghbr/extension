...
async function handleAutocompleteRequest(
  ...
  if (!stored["vigogh-auth-token"]) {
    return { success: false, error: "No auth token", reason: "unauthenticated" };
  }

  try {
    const { data } = await api.post(getEndpoint("autocomplete"), { text, url, messages });
    return {
      success: true,
...
export const handleMessages: BackgroundMessageHandler = (message, _sender, sendResponse) => {
  if (message.action === "autocomplete_request") {
    handleAutocompleteRequest(message.text, message.url, (message as { messages?: { role: string; text: string }[] }).messages || [])
      .then(sendResponse)
